...
<h1 style="font-weight:normal;font-family:arial;">
	...
	{if $order->status != 0 || $order->paid == 1}
		{if $order->total_price>0}на сумму {$order->total_price|convert:$currency->id}&nbsp;{$currency->sign}{/if}
...
<table cellpadding="6" cellspacing="0" style="border-collapse: collapse;">
	...
	<tr>
		<td align="center" style="padding:6px; width:100; padding:6px; background-color:#ffffff; border:1px solid #e0e0e0;font-family:arial;">
			...
			<a href="{$config->root_url}/products/{$purchase->product->url}">{if $image}<img border="0" src="{$image->filename|resize:50:50}">{/if}</a>
		</td>
		...
		<td align=right style="padding:6px; text-align:right; width:150; background-color:#ffffff; border:1px solid #e0e0e0;font-family:arial;">
			{$purchase->amount} {$settings->units} {if $purchase->price>0}&times; {$purchase->price|convert:$currency->id}&nbsp;{$currency->sign}{else}{assign var='has_requests' value=1}{/if}
		</td>
	</tr>
	{/foreach}
	...
	<tr>
		...
		<td align="right" style="padding:6px; text-align:right; width:170; background-color:#ffffff; border:1px solid #e0e0e0;font-family:arial;font-weight:bold;">
			{if $has_requests}от&nbsp;{/if}{$order->total_price|convert:$currency->id}&nbsp;{$currency->sign}
		</td>
	</tr>
	{/if}
</table>
